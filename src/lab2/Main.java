...
class Game extends BaseAppState {
    ...
    protected void initialize(Application app) {
        ...
        DirectionalLightShadowRenderer dlsr = new DirectionalLightShadowRenderer(sapp.getAssetManager(), SHADOWMAP_SIZE, 3);
        dlsr.setLight(sun);
        dlsr.setShadowIntensity(0f);
        sapp.getViewPort().addProcessor(dlsr);
        DirectionalLightShadowFilter dlsf = new DirectionalLightShadowFilter(sapp.getAssetManager(), SHADOWMAP_SIZE, 3);
        dlsf.setLight(sun);
        dlsf.setEnabled(true);
        FilterPostProcessor fpp = new FilterPostProcessor(sapp.getAssetManager());
        fpp.addFilter(dlsf);
        sapp.getViewPort().addProcessor(fpp);       
    }
    ...
    protected void onEnable() {
        if (needCleaning) {
            ...
            sapp.getGameNode().detachAllChildren();
        ...
        sapp.getGameNode().attachChild(gb.getGameBoard());                 
            ...
            for (int j=0; j<5 ;j++){
                if(!((i>0 && i<4)&&(j>0 && j<4))){
                    if (isNeg){
                        ...
                        sapp.getStateManager().attach(disk);
                        diskStore.add(disk);
                    }else{
    ...
    private void newPlayer(int i){
        ...
        sapp.getStateManager().attach(player); 
    ...
    private void initHUD(){
        ...
        sapp.getGuiNode().attachChild(timeAndScore); 
    }
...
class Ask extends BaseAppState {
    ...
    protected void onEnable() {
        ...
        hudText.setLocalTranslation(5, 200, 0);
...
abstract class Disk extends BaseAppState {
    ...
    public Disk(String type, ColorRGBA color, float radius, float size, float X_pos, float Y_pos){
        this.COLOR = color;
        this.RADIUS = radius;
        this.SIZE = size/2;
        this.X_POS = X_pos;
    ...
    protected void initialize(Application app) {
        ...
        mat_disk = new Material(simpleApp.getAssetManager(), "Common/MatDefs/Misc/Unshaded.j3md");
        ...
        simpleApp.getGameNode().attachChild(node_disk); 
    }  
...
class Player extends Disk {    
    private final KeyTrigger UP;
    private final KeyTrigger DOWN;
    private final KeyTrigger RIGHT;
    private final KeyTrigger LEFT;
    private int id;
    private final int SPEED_ACCELERATION = 60; 
    private static int playerNumber = 0;
      
    public Player(float X_pos, float Y_pos, KeyTrigger up, KeyTrigger down, KeyTrigger right, KeyTrigger left){
        super("X", ColorRGBA.Blue, Game.PLAYER_R, Game.FRAME_THICKNESS, X_pos, Y_pos);
        this.UP = up;
        this.DOWN = down;
        this.LEFT = left;
        this.RIGHT = right; 
    ...
    protected void initialize(Application app) {        
        ...
        //init key :
        simpleApp.getInputManager().addMapping("UP"+id, this.UP);
        simpleApp.getInputManager().addMapping("DOWN"+id, this.DOWN);
        simpleApp.getInputManager().addMapping("LEFT"+id, this.LEFT);
        simpleApp.getInputManager().addMapping("RIGHT"+id, this.RIGHT);
        simpleApp.getInputManager().addListener(analogListener, "UP"+id, "DOWN"+id, "LEFT"+id,"RIGHT"+id);
    ...
    private void newPlayerMarker(int playerID){        
        ...
        Material mat_id = new Material(simpleApp.getAssetManager(), "Common/MatDefs/Misc/Unshaded.j3md");
    ...
    protected void onDisable() {
        ...
        // delete mapping :
        simpleApp.getInputManager().deleteMapping("UP"+id); 
        simpleApp.getInputManager().deleteMapping("DOWN"+id);
        simpleApp.getInputManager().deleteMapping("LEFT"+id);
        simpleApp.getInputManager().deleteMapping("RIGHT"+id);
    ...
    private AnalogListener analogListener = new AnalogListener() {
        ...
        public void onAnalog(String name, float value, float tpf) {
            if (isEnabled()){
                ...
                if (name.equals("UP"+id)){
                    Y_SPEED += SPEED_ACCELERATION * tpf;
                }
                if (name.equals("DOWN"+id)){
                    Y_SPEED -= SPEED_ACCELERATION * tpf; 
                }
                if (name.equals("LEFT"+id)){
                    X_SPEED -= SPEED_ACCELERATION * tpf;
                }
                if (name.equals("RIGHT"+id)){
                    X_SPEED += SPEED_ACCELERATION * tpf;
                }
            }
...
class NDisk extends Disk {
    ...
    NDisk(float X_pos, float Y_pos){
        super("N", ColorRGBA.Red, Game.NEGDISK_R, Game.FRAME_THICKNESS, X_pos, Y_pos);
...
class PDisk extends Disk {
    ...
    PDisk(float X_pos, float Y_pos){
        super("P", ColorRGBA.Green, Game.NEGDISK_R, Game.FRAME_THICKNESS, X_pos, Y_pos);
        this.X_SPEED = -INIT_SPEED_VALUE + (float)Math.random()*2*INIT_SPEED_VALUE;
        this.Y_SPEED = -INIT_SPEED_VALUE + (float)Math.random()*2*INIT_SPEED_VALUE;
        this.POINT = 5;
    }
    ...
    protected void initialize(Application app) {
        ...
        for (int i = 0; i < 5; i++){            
            ...
            Material mat_point = new Material(simpleApp.getAssetManager(), "Common/MatDefs/Misc/Unshaded.j3md");
